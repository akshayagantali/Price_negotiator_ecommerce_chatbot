...
ds = pd.read_csv(
    os.path.join(BASE_DIR, "recommendation", "WebProducts.csv")
)
...
'''
    ...
    ============================================
''' 
#graph = tflow.compat.v1.get_default_graph()  # Not needed in eager mode
...
data = pickle.load(
    open(
        os.path.join(BASE_DIR, "..", "Ecom_chatbot v0.3", "training_data"),
        "rb"
    )
)
words = data['words']
...
# load intents
with open(os.path.join(BASE_DIR, "..", "Ecom_chatbot v0.3", "intent.json")) as json_data:
    intents = json.load(json_data)

# load the model
model = load_model(
    os.path.join(BASE_DIR, "..", "Ecom_chatbot v0.3", "chatmodel.h5"),
    compile=False
)
...
def classify(sentence):
    ...
    # reshape to (1, 1, vocab_size) for the model input
    tot = np.array(p, dtype=np.float32).reshape(1, 1, len(p))
    
    # Use model directly (eager mode compatible)
    results = model(tot, training=False).numpy()[0]
...
def response(sentence, userID, min_acceptable_price, user_offer, original_price, show_details=False):
    ...
    # Fallback: if model did not classify but message contains rs. amount, treat as customeroffers
    import re
    has_price = bool(re.search(r'rs\.\s*\d+', sentence.lower()))
    if not results and has_price and isinstance(user_offer, int):
        results = [('customeroffers', 1.0)]
                    ...
                    if not 'context_filter' in i or \
                        ...
                        # if the user accepts the deal offered by the bot
                            ...
                            if 'discount_price' not in session:
                                # use bot_offer if available, else use min_acceptable_price
                                session['discount_price'] = session.get('bot_offer', min_acceptable_price)
                        ...
                        res = random.choice(i['responses'])
                        # replace ___ placeholder if present (e.g. finaloffer tag)
                        if '___' in res:
                            bot_offer = session.get('bot_offer', min_acceptable_price)
                            res = res.replace('___', str(bot_offer))
                        return res
            results.pop(0)
...
def get_bot_response():
    ...
    # --- KEYWORD BYPASS (works without model retraining) ---
    # 1. Detect deal/acceptance directly from keywords
    deal_keywords = ['add to cart', 'add it to cart', 'add it to the cart', 'book it',
                     'confirm', 'place the order', 'okay add', 'ok add', 'yes add',
                     'book it for', 'add the product', 'i accept', 'deal', 'accepted',
                     'done deal', 'we have a deal', 'i agree', 'agreed']
    is_deal_message = any(kw in userTextLower for kw in deal_keywords)

    # 2. Detect price offers directly from rs. pattern
    import re as _re
    price_match = _re.search(r'rs\.\s*(\d+)', userTextLower)
    user_offer = None if is_deal_message else extract_amount(userText)

    # 3. If message contains rs.NUMBER, treat as price offer bypass (no model needed)
    if price_match and not is_deal_message:
        prd_id = session.get('prd')
        if prd_id is None:
            return "Please open a product first before bargaining."
        original_price, min_acceptable_price = get_product_info(prd_id)
        user_offer = int(price_match.group(1))
        track_user_offer(userText)

        if user_offer > original_price or user_offer == 0:
            # foolish offer
            foolish_responses = [
                "i am a honest bot i will not accept prices that are not sensible",
                "that is not a sensible offer",
                "i cannot accept that",
                "that would be silly to offer such price"
            ]
            return random.choice(foolish_responses)
            ...
            accept_responses = [
                f"your offer seems reasonable. we have a deal on rs.{user_offer}/- You may now add the product to cart.",
                f"I accept your offer at rs.{user_offer}/- You may now add the product to cart.",
                f"ok! we have a deal at rs.{user_offer}/- click on add to cart button now"
            ]
            ...
            if bot_offer_count > 3:
                return f"full and final offer is rs.{bot_offer}/- I will not go lower than that"

            counter_responses = [
                f"I will give it out for Rs.{bot_offer}/-",
                f"Alright then give Rs.{bot_offer}/- for it.",
                f"for you i will sell it in Rs.{bot_offer}/-",
                f"Although the price is fair but i will reduce it to rs.{bot_offer}/- for you",
                f"that would be a steal lets close the deal on rs.{bot_offer}/-"
            ]
            return random.choice(counter_responses)

    # 4. If it is a deal message, handle immediately without model
    if is_deal_message:
        prd_id = session.get('prd')
        if prd_id is None:
            return "Please open a product first before bargaining."
        if 'discount_price' not in session:
            bot_offer = session.get('bot_offer')
            if bot_offer:
                session['discount_price'] = bot_offer
                session.pop('bot_offer', None)
            elif price_match:
                session['discount_price'] = int(price_match.group(1))
        deal_responses = [
            "Thanks! we have a deal. Please click on add to cart button now",
            "Thank you for your time. You can now add the item to cart to avail discounted price",
            "Thank you. Click on add to cart button to avail the discounted price"
        ]
    ...
    try:
        ...
        elif user_offer == 'invalid_amount':
            res = response("bla bla", '123', min_acceptable_price, user_offer, original_price)
        elif res == 'suitable offer by the customer':
            res = response('suitable offer by the customer', '123', min_acceptable_price, user_offer, original_price)
            if res and '___' in res:
                res = res.replace('___', str(user_offer))
            if 'discount_price' not in session:
                session['discount_price'] = user_offer
        elif res == 'foolish offer':
            res = response('foolish offer', '123', min_acceptable_price, user_offer, original_price)
        elif isinstance(res, str) and 'final offer+' in res:
            res_split = res.split('+')
            final_offer = res_split[1]
            res = response('final offer', '123', min_acceptable_price, user_offer, original_price)
            if res and '___' in res:
                res = res.replace('___', str(final_offer))
            if 'discount_price' not in session:
                session['discount_price'] = int(final_offer)
    ...
    return str(res) if res else "Sorry, can't understand you"
